...
| `src/core/heartbeatTools.ts` | Heartbeat エージェント用ツール定義（listCalendarEvents, getCurrentTime, fetchFeeds, listFeeds, checkMonitors） |
...
```
server/
├── wrangler.toml       # Cron: */15 * * * *, KV binding
├── package.json
├── tsconfig.json
├── scripts/
