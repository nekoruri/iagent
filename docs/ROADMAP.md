...
- [x] 記憶品質ガバナンス（PoC-7 Phase1）— MemoryPanel で記憶の編集（内容/重要度/タグ）と手動無効化（アーカイブ）を追加。`memoryStore` に更新 API と再評価候補抽出（低重要度かつ長期間未参照）を追加し、UI 上で「再評価候補」件数と対象バッジを表示。（2026-03-05）
