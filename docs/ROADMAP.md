...
- [x] オンボーディング計測拡張（PoC-3 follow-up）— SetupWizard 操作を `setup-wizard` ops-event として記録（開始/遷移/スキップ/プリセット適用/完了）。`metrics:poc` に完了率・完了時間中央値・推奨プリセット採用率を追加し、週次レビュー/ baseline テンプレートへ自動反映。（2026-03-05）
