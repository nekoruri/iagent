...
export default defineConfig(({ mode }) => {
  ...
  const heartbeatOpenAiUrl = env.VITE_OPENAI_API_URL || 'https://api.openai.com/v1/chat/completions'

  return {
    define: {
      __HEARTBEAT_OPENAI_API_URL__: JSON.stringify(heartbeatOpenAiUrl),
    },
    plugins: [
      react(),
      cspPlugin(),
